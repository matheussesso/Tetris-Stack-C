// TETRIS STACK - FILA + PILHA COM TROCAS (Nivel Mestre)
// Gerenciador avancado com trocas simples e multiplas entre estruturas
...
void trocar_peca_atual() {
    if (pilha.topo == -1) {
        printf("\nErro: Pilha vazia! Nao ha peca para trocar.\n");
        return;
    }
    
    Peca peca_fila = fila.itens[fila.inicio];
    Peca peca_pilha = pilha.itens[pilha.topo];
    ...
    printf("╚════════════════════════════════════════════╝\n");
    printf("Peca da fila (saiu):   %c %d\n", peca_fila.nome, peca_fila.id);
    printf("Peca da pilha (entrou): %c %d\n", peca_pilha.nome, peca_pilha.id);
}

// Troca as 3 primeiras pecas da fila com as 3 da pilha
void trocar_multipla() {
    // Verifica se ha 3 pecas na pilha
    if (pilha.topo < 2) {
        printf("\nErro: Pilha nao possui 3 pecas para trocar!\n");
        return;
    }
    
    // Verifica se ha 3 pecas na fila
    if (fila.quantidade < 3) {
        printf("\nErro: Fila nao possui 3 pecas para trocar!\n");
        return;
    }
    ...
    // Salva as 3 pecas da fila em um array temporario
    Peca temp_fila[3];
    for (int i = 0; i < 3; i++) {
        int indice = (fila.inicio + i) % TAMANHO_FILA;
        temp_fila[i] = fila.itens[indice];
        printf("Peca fila %d (saiu): %c %d\n", i + 1, temp_fila[i].nome, temp_fila[i].id);
    }
    
    // Move as 3 pecas da pilha para a fila
    for (int i = 0; i < 3; i++) {
        int indice = (fila.inicio + i) % TAMANHO_FILA;
        fila.itens[indice] = pilha.itens[pilha.topo - i];
        printf("Peca pilha %d (entrou): %c %d\n", i + 1, fila.itens[indice].nome, fila.itens[indice].id);
    }
    
    // Move as pecas salvas da fila para a pilha (invertendo a ordem)
    for (int i = 0; i < 3; i++) {
        pilha.itens[2 - i] = temp_fila[i];
    }
}
...
void exibir_menu() {
    ...
    printf("║ 4 - Trocar peca da fila com a pilha      ║\n");
...
int main() {
    ...
    printf("\n");
    printf("╔════════════════════════════════════════════╗\n");
    printf("║     TETRIS STACK - FILA + PILHA + TROCAS  ║\n");
    printf("║        Nivel Mestre - FIFO + LIFO        ║\n");
    printf("╚════════════════════════════════════════════╝\n");
